...
class DashboardStats extends BaseWidget
{
    ...
    protected function getStats(): array
    {
        ...
        $totalPoints = Point::where('point_type', 'earned')
            ->whereBetween('point_date', [$startDate, $endDate])
            ->sum('point_value');
        ...
        return [
            Stat::make('Total Users', User::whereBetween('registration_date', [$startDate, $endDate])->count())
                ->description('Active loyalty program members')
                ->descriptionIcon('heroicon-m-user-group')
                ->chart(User::select(DB::raw('DATE(registration_date) as date'), DB::raw('count(*) as count'))
            ...
            Stat::make('Total Transactions',
                Transaction::whereBetween('transaction_date', [$startDate, $endDate])->count())
                ->description('Total value: ₦' . number_format(
                    Transaction::whereBetween('transaction_date', [$startDate, $endDate])->sum('transaction_amount'),
                    2
                ))
            ...
            Stat::make('Points Balance', number_format($totalPoints - $redeemedPoints))
                ...
                ->chart(Point::select(DB::raw('DATE(point_date) as date'), DB::raw('sum(point_value) as total'))
